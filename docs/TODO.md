...
| 12 | 动态地图分布规则第一版（饭点/夜间回流/阵营偏好/玩家位置修正） | L5 | architecture | 180m | 🔴 next |

**执行顺序**：L5 当前先完成地图动态分布，再回头补季节/节日等内容驱动层，最后进入调教与更重的系统入口。
...
| **L5** 🟡 | 推进中 | 88% | 背包 ✅ / 誓约底座 ✅ / 商店闭环 ✅ / 皮肤与外观底座 ✅ / 时间与日历底座 ✅ / 季节规则接入 📋 / 地图分层基础 ✅ |

*标注：✅完成 🔄进行中 🟡部分完成 📋筹划中 ⬜待启动*

---

## 🔴 当前活跃任务（主线 G：L5 动态地图分布规则第一版）

> 地图分层基础已经落地，时间系统、工作安排、委托隐藏和地点 roster 也都已经有了可复用底座。下一步不宜继续堆内容，而应先把“非工作状态舰娘如何在地图上轻量流动”这层规则补齐，让地图、时间、日历、关系和后续节日内容都挂到同一套空间语义上。
...
- [ ] 动态地图分布规则第一版（饭点/夜间回流/阵营偏好/玩家位置修正）
	- type: architecture
	- priority: P0
	- milestone: L5
	- est: 180m
	- DoD: 非强制状态角色可基于当前时间、工作安排、宿舍归属、活动标签和玩家位置得到稳定地点分布；饭点采用“基础饭点 + 角色级轻微偏移”以减少餐饮区拥挤；委托/同行/约会仍保持最高优先级；地图 roster 与自动选中继续兼容
	- verify: `python -m unittest tests.test_distribution tests.test_navigation tests.test_time_system tests.test_cli_selection -v`
	- owner: pair
	- status: next
	- updated: 2026-04-18
...
- **本周完成**：L5 道具底座（`inventory` / `items.toml` / `required_items`）；`ShopService` + `shopfronts.toml`；CLI 最小商店入口；`oath_success` / `oath_failure` 结果标签与企业差分口上；皮肤与外观底座（内容定义 / 存档 / skin shop / 衣柜 / oath reward / 皮肤分支）；时间与日历底座（真实日期 / 24 小时时钟 / 固定分钟耗时 / 节日 / 排班 / 日历指令）；地图分层基础（大区 / 子区 / 槽位 / 角色归属 / 在场分布 / CLI 分页选中）；全量回归
- **本周阻塞**：无
- **指标**：
	- 自动化测试通过率：390/390（100%）
	- 正式角色总数：3
	- 指令总数：54+，含工作与恢复指令
	- 新增 L5 完成项：背包、誓约底座、最小道具门禁、皮肤与外观底座、时间与日历底座、地图分层基础
...
**即将开始**：L5 动态地图分布规则（饭点 / 夜间回流 / 阵营偏好 / 玩家位置修正）  
**后续规划**：季节与节日规则接入 + 阵营生活区扩充 + 地图 / NPC 商店入口  
**长期计划**：商店 NPC 化（明石/不知火）+ 地图 UI 深化 + 调教系统 + 交互升级
