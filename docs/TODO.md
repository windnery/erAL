...
| 6 | 季节与节日规则接入（季节系统 + 扩展接口） | L5 | content | 90m | ✅ done |
...
| 21 | 记忆与解锁追踪系统 | L7 | architecture | 120m | ✅ done |
| 22 | 调教指令全量补齐（93条训练指令） | L7 | content | 180m | ✅ done |
| 23 | 调教指令数值审查与修正 | L7 | gameplay | 60m | ✅ done |
...
| **L7** ✅ | 完成 | 100% | 记忆与解锁系统 ✅ / 调教指令全量补齐 ✅ / 数值审查修正 ✅ |

*标注：✅完成 🔄进行中 🟡部分完成 📋筹划中 ⬜待启动*

---

## 🔴 当前活跃任务（主线：L8 内容深化与系统打磨）

> L7 全部完成（记忆系统 + 93条调教指令全量补齐 + 数值审查，530 项测试通过）。
> 下一步：L8 内容深化与系统打磨。
...
- [ ] 调教口上内容编辑器优化（批量生成/模板/预览）
	- type: tooling
	- priority: P2
	- milestone: L8
	- est: 120m
	- DoD: 编辑器支持批量导入调教口上模板，可按指令预览
	- est: 120m
	- DoD: 商店入口从地点挂载改为角色 NPC 入口
...
- [x] 通用商店骨架能复用到 `general_shop` 与 `skin_shop`
- [x] 角色皮肤可被购买、切换并进入存档
...
**内容量**
- [x] 153 个指令已接入 `commands.toml`（含 93 条训练指令）
...
- [x] 记忆与解锁系统（memories 字典 + cmd:/evt: 自动记录 + 口上/事件查询）
- [x] 调教指令全量补齐（93 条训练指令，含体位差分/道具/SM/被动/状况系）
- [x] 调教数值审查修正（V插入 pleasure 修正 / 5 条 settlement 规则补齐 / display_name 修正）
- [x] 全量 fallback 口上（93 条训练指令 100% 覆盖）
- [x] 全量 DEV_MAP 映射（50+ 条开发度追踪）
- [x] 自动化测试 530 项通过
...
- **本周完成**：L7 记忆与解锁系统（memories 字典 + 自动记录 + 口上/事件查询）；调教指令全量补齐（从 21 条扩展到 93 条，覆盖体位差分/道具/SM/被动/状况系）；调教数值审查修正（V插入 pleasure_b→pleasure_v 修复、5 条 settlement 规则补齐、display_name 修正、train_rape 数值再平衡）；全量 fallback 口上（93 条 100% 覆盖）+ DEV_MAP 映射（50+ 条）；自动化测试 530 项通过
- **本周阻塞**：无
- **指标**：
	- 自动化测试通过率：530/530（100%）
	- 正式角色总数：3
	- 指令总数：153，含 93 条训练指令
	- 新增 L7 完成项：记忆系统、调教指令全量补齐、数值审查修正
...
**即将开始**：L4 稳定性工程 / L5+ 调教系统骨架（重樱 / 铁血 / 东煌 / 混合）
**后续规划**：调教系统骨架 + 内容校验增强 + 角色内容扩充
