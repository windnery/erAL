...
> 路径引擎 ✅ + 阵营扩充 ✅ + 分布 v2 ✅ 已落地：`PortConnection` 支持 `cost_minutes` 差异化边权；`PortMap` 内置 Dijkstra 最短路径与可达目的地查询；`NavigationService` 重构为 `plan_move` / `available_destinations` / `execute_move` 三层，返回 UI 无关的 `MovePlan` 结构化数据；CLI 移动菜单已改为按大区分组显示所有可达地点及耗时。下一步推进 ASCII 地图整理（确认文件格式 + 拆分区域 Map 文件），为后续可视化做准备。
