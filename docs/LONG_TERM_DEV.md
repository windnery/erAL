...
| 维度 | 状态 |
|------|------|
| 测试 | 全量回归入口可用，新增 vital 专项测试（全量通过率待本轮复验） |
| 内容校验 | 通过 |
| 角色包 | 3 (企业/拉菲/标枪) |
| 指令数 | 54 + recovery 类扩展（含 sleep/nap/bathe，待去重校验） |
| 地点数 | 10 |
...
### 5.1 L3 数值闭环与MVP打磨（当前）

| 任务 | 类型 | 预估 | 依赖 | 状态 |
|------|------|------|------|------|
| 体力/气力首轮平衡表（五类动作） | 数值 | 1 周 | specs 与 commands 对齐 | 进行中 |
| 修复 recovery 指令 key 冲突与命令一致性 | 代码 | 0.5 天 | commands 校验 | 待办 |
| VitalGate/晕倒推进/恢复链路 E2E 回归 | 测试 | 1 周 | test_vitals + smoke | 待办 |
| 关系成长公式与门槛调参说明 | 文档 | 0.5 天 | growth/stages 配置 | 待办 |
| 连续 14 天可玩烟测 | 测试 | 1 周 | 上述任务完成后 | 待办 |
| 誓约层指令 | 内容 + 代码 | 数值标定 | 待办 |
| 誓约层事件 | 内容 | 指令定义 | 待办 |
| 誓约层对话 | 内容 | 事件定义 | 待办 |
| 通用口上兜底 | 内容 | 54指令 | 待办 |
