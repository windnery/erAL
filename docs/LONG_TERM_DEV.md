...
L3 数值闭环与MVP打磨 ──── 已完成 ✓
│  ├── 体力/气力系统稳定化 (已接入骨架，待平衡)
│  ├── 关系成长公式与阶段门槛调参
│  ├── 誓约层内容 (指令+事件+对话)
│  ├── CLI 体感优化 (已完成主界面+能力显示升级)
│  └── 14天可玩烟测 (待执行)

L4 内容规模化 ─────────── 进行中
│  ├── 经济循环 F1-F3 完成（WalletService + 工作 + 委托）
│  ├── 港区开发系统 F4（待实现）
│  ├── 角色量产 (6-12 新角色)
│  ├── 关系阶段深度扩展
│  ├── 地点玩法独立化
...
L5 玩法系统扩展 ──────── 规划中
│  ├── 体力系统高级联动 (天气/家具/道具/场景倍率)
│  ├── 天气/季节系统
│  ├── ABL 技能树
│  └── 深层亲密系统 (H)

L6 表现层 ────────────── 规划中
│  ├── UI 升级 (TUI 基础已完成，Web 待规划)
│  └── 音效/立绘系统
```
...
| 维度 | 状态 |
|------|------|
| 测试 | 606 全通过，含 L8 反击/射精/随机事件/跨区流动 |
| 内容校验 | 通过 |
| 角色包 | 3 (企业/拉菲/标枪) |
| 指令数 | 160（含 93 条训练指令 + 射精切换） |
| 地点数 | 10 |
| 关系阶段 | 5 |
| 通用口上 | 100% 指令 fallback 覆盖 |
| 港区设施 | 3 (官舍/食堂/约会点，各4级) |
| 持续状态 | 13 (6 身体槽位 + 4 装备槽位 + 3 无槽位) |
| 赠礼 | 10 种 |
| 随机事件池 | 8 条 ambient 事件 |
| CLI UI | 主界面 PALAM 等级条 + tab 切换 + 能力显示 5tab |
...
### 4.2 关系阶段门槛（当前实现，持续调参）

```
stranger → friendly: affection≥200, trust≥100
friendly → like:    affection≥400, trust≥200, intimacy≥3
like → love:       affection≥800, trust≥400, intimacy≥5, 无 dislike_mark
love → oath:       affection≥2000, trust≥1000, intimacy≥8, 无 dislike_mark, 需 pledge_ring
```

### 4.3 体力/精力消耗（已接入骨架，待平衡）

设计规格： [体力与气力系统](specs/体力与气力系统.md)

当前已落地：

1. 指令 `downbase` 消耗通路
2. `VitalService`（自然恢复、sleep/nap/bathe 恢复、fatigue 累积）
3. `VitalGate`（气力/体力门槛）
4. 体力归零后晕倒并推进到次日 dawn

L3 待完成范围：

1. 指令分类消耗
2. 休憩/睡眠/道具恢复数值平衡
3. 低体力/低气力门槛文案与可用性一致性
4. 天赋、家具、场景倍率
5. 疲劳返家与行动中断
...
### 5.1 L3 数值闭环与MVP打磨（收尾中）

| 任务 | 类型 | 预估 | 依赖 | 状态 |
|------|------|------|------|------|
| 体力/气力首轮平衡表（五类动作） | 数值 | 1 周 | specs 与 commands 对齐 | **已完成** |
| 修正恢复指令 key 冲突与命令一致性 | 代码 | 0.5 天 | commands 校验 | **已完成** |
| VitalGate/晕倒推进/恢复链路 E2E 回归 | 测试 | 1 周 | test_vitals + smoke | **已完成** |
| CLI 主界面状态面板升级 | 代码 | — | 无 | **已完成** |
| 关系成长公式与门槛调参说明 | 文档 | 0.5 天 | growth/stages 配置 | 待办 |
| 连续 14 天可玩烟测（基础版） | 测试 | 1 周 | 上述任务完成后 | 待办 |
| 连续 14 天可玩烟测（全链路版） | 测试 | 1 周 | 基础版通过 | 待办 |
| 誓约层指令+事件+对话 | 内容+代码 | 数值标定 | 待办 |
| 通用口上兜底（54 指令） | 内容 | — | 待办 |
| 日终反馈摘要 | 代码 | 无 | 待办 |
...
| 任务 | 类型 | 预估 | 依赖 | 状态 |
|------|------|------|------|------|
| 资金与账本系统（WalletService + 双账户） | 代码 | — | — | **已完成** |
| 工作系统（office_shift / extra_shift） | 代码 | — | 资金系统 | **已完成** |
| 委托系统（CommissionService 5种委托） | 代码 | — | 资金系统 | **已完成** |
| 港区开发系统（FacilityService） | 代码 | 1 周 | 经济循环 F1-F3 | **下一阶段** |
| 贝尔法斯特角色包 | 内容 | — | L4前期 | 待办 |
| 光辉角色包 | 内容 | — | L4前期 | 待办 |
| 俾斯麦角色包 | 内容 | — | L4前期 | 待办 |
| 皇家方舟角色包 | 内容 | — | L4前期 | 待办 |
| 独角兽角色包 | 内容 | — | L4中期 | 待办 |
| 地图区域化重构 | 架构 + 内容 | — | — | 待办 |
| 地点扩展 (10→30) | 内容 | — | 区域化 | 待办 |

### 5.3 L5 玩法系统扩展

| 任务 | 类型 | 预估 | 依赖 | 状态 |
|------|------|------|------|------|
| 体力系统高级联动（天气/家具/道具/场景倍率） | 代码 | L4完成 | 待办 |
| 天气系统 (TFLAG) | 代码 | 无 | 待办 |
| ABL 技能树 | 代码 | 体力高级联动 | 待办 |
| PALAM 衰减 | 代码 | 体力高级联动 | 待办 |
...
| 任务 | 类型 | 预估 | 依赖 | 状态 |
|------|------|------|------|------|
| TUI 基础（CLI 主界面 + 能力显示） | 代码 | — | — | **已完成** |
| TUI 进阶（日终摘要/更多交互提示） | 代码 | — | TUI基础 | 待办 |
| Web UI | 代码 | — | L4完成 | 待办 |
| 音效系统 | 资产 | — | — | 待办 |
| 立绘系统 | 代码 + 资产 | — | L5中期 | 待办 |
