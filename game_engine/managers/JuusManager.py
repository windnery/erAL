...
class JuusManager:
    ...
    def get_contacts_list(self) -> list[dict[str, Any]]:
        ...
        for sg in self.world.npc_manager.get_all_npcs():
            ...
            n_name = self.world.map_manager.maps.get(r_key, {}).get(n_key, {}).get('name', n_key)
            ...
            rel_val = str(sg.get_talent_value('relationship'))
    ...
    def get_contact_detail(self, shipgirl_id: str) -> dict[str, Any] | None:
        ...
        for w in works:
            ...
            w_r = self.world.map_manager.regions.get(w['location']['region'], {}).get('name', '')
            w_n = self.world.map_manager.maps.get(w['location']['region'], {}).get(w['location']['node'], {}).get('name', '')
            work_entries.append(f"{w_start[0]:02d}:{w_start[1]:02d} ~ {w_end[0]:02d}:{w_end[1]:02d} {w_desc}（{w_r}-{w_n}）")
        ...
        contact_base['is_current_location'] = (
            player.location.get('region') == sg.location.get('region') and
            player.location.get('node') == sg.location.get('node')
        )
