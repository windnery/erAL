...
from game_engine.commands._common import data_score_calc
...
def end_date(world: World, option: str, time_out: bool = False):
    ...
    if time_out:
        ...
        ctx.say(f'结束了和{npc.name}的约会……')

    # 计算约会氛围得分
    score = data_score_calc(npc)

    # 触发约会结束事件调度（初吻 / 反发消除 / 告白 / 通常道别）
    world.event_manager.trigger(
        EventTrigger.DATE_END,
        ctx=ctx,
        target=npc,
        score=score,
        time_out=time_out,
