...
@register_cmd('spread_the_labia', '张开阴唇', cat='爱抚', train_mode=True, can=can, needs_target=False)
def spread_the_labia(world: World):
    ...
    for target_id in train.targets:
        ...
        if target_id != PLAYER_ID:
            # 只有舰娘有口上
            line = chara.get_line('spread_the_labia')  # type: ignore
            if line:
                # 有口上
    ...
    ctx.say(f'度过了{command_time_data["spread_the_labia"]}分钟')
    return ctx.result()
