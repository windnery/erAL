...
def can(world: World):
    ...
    if len(train_manager.train.actors) * 2 < len(train_manager.train.targets):  # type: ignore
        return False
    # 槽位判定（被调教方1手1阴道）
    if not check_body_slots(world, actor_slots={}, target_slots={'hands': 1, 'vagina': 1}):
        return False
...
@register_cmd(
    ...
    needs_target=False,
    continuous=True,
    continuous_text='{actors}命令{targets}张开阴唇',
    actor_slots={},
    target_slots={'hands': 1, 'vagina': 1},
    continuous_tick=continuous_tick,
)
def spread_the_labia(world: World):
    """张开阴唇"""
    ...
    if getattr(world, 'is_current_cmd_continuous', False):
        new_cmd = world.train_manager.add_continuous_cmd(
            'spread_the_labia', list(train.actors), list(train.targets))
        if new_cmd:
    ...
    if tar_num > 1:
        tar_name += '等人'
    ctx.say(f'{src_name}命令{tar_name}张开阴唇……')
    for target_id in train.targets:
        chara = get_entity_by_id(world.player, target_id)
        if target_id != PLAYER_ID:
            # 只有舰娘有口上
            say_chara_line(chara, ctx, 'spread_the_labia')

    # 推进时间
    ctx.advance_time(command_time_data['spread_the_labia'])
