...
def can(world: World):
    ...
    if not train_global_can(train_manager):
        return False
    # 槽位判定（调教方1嘴，被调教方1菊穴）
    if not check_body_slots(world, actor_slots={'mouth': 1}, target_slots={'ass': 1}):
        return False
...
@register_cmd(
    ...
    needs_target=False,
    continuous=True,
    continuous_text='{actors}正在舔舐{targets}的肛门',
    actor_slots={'mouth': 1},
    target_slots={'ass': 1},
    continuous_tick=continuous_tick,
)
def lick_ass(world: World):
    """舔肛"""
    ...
    if getattr(world, 'is_current_cmd_continuous', False):
        new_cmd = world.train_manager.add_continuous_cmd(
            'lick_ass', list(train.actors), list(train.targets))
        if new_cmd:
    ...
    if tar_num > 1:
        tar_name += '等人'
    ctx.say(f'{src_name}把脸埋在了{tar_name}的菊穴上，用舌头激烈地舔舐着……')
    for target_id in train.targets:
        chara = get_entity_by_id(world.player, target_id)
        if target_id != PLAYER_ID:
            # 只有舰娘有口上
            say_chara_line(chara, ctx, 'lick_ass')

    # 推进时间
    ctx.advance_time(command_time_data['lick_ass'])
    ...
    for actor_id in train.actors:
        ...
        chara = get_entity_by_id(world.player, actor_id)
        # abl: 舌
        temp_sources[actor_id]['a_pleasure_source'] += chara.abl['tongue_abl'] * 2
