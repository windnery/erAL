...
def continuous_tick(world: World, ctx: CommandContext, cmd: ContinuousCommand):
    ...
    for target_id in cmd.target_ids:
        ...
        # 50% 消耗
        ctx.consume(stamina=5, energy=25, chara=chara)
...
@register_cmd(
    ...
    needs_target=False,
    continuous=True,
    continuous_text='{actors}正在揉捏{targets}的胸部',
    actor_slots={'hands': 1},
...
def breast_massage(world: World):
    ...
    if getattr(world, 'is_current_cmd_continuous', False):
        new_cmd = world.train_manager.add_continuous_cmd(
            'breast_massage', list(train.actors), list(train.targets))
        if new_cmd:
