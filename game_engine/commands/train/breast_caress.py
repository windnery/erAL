...
@register_cmd('breast_caress', '胸爱抚', cat='爱抚', train_mode=True, can=can, needs_target=False)
def breast_caress(world: World):
    ...
    for target_id in train.targets:
        ...
        if target_id != PLAYER_ID:
            # 只有舰娘有口上
            line = chara.get_line('breast_caress')  # type: ignore
            if line:
                # 有口上
    ...
    ctx.say(f'度过了{command_time_data["breast_caress"]}分钟')
    return ctx.result()
