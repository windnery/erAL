...
def can(world: World):
    ...
    if len(train_manager.train.actors) * 2 < len(train_manager.train.targets):  # type: ignore
        return False
    # 槽位判定（调教方1手，被调教方1胸）
    if not check_body_slots(world, actor_slots={'hands': 1}, target_slots={'breasts': 1}):
        return False
...
@register_cmd(
    ...
    needs_target=False,
    continuous=True,
    continuous_text='{actors}正在爱抚{targets}的胸部',
    actor_slots={'hands': 1},
    target_slots={'breasts': 1},
    continuous_tick=continuous_tick,
)
def breast_caress(world: World):
    """胸爱抚"""
    ...
    if getattr(world, 'is_current_cmd_continuous', False):
        new_cmd = world.train_manager.add_continuous_cmd(
            'breast_caress', list(train.actors), list(train.targets))
        if new_cmd:
    ...
    for target_id in train.targets:
        ...
        if target_id != PLAYER_ID:
            # 只有舰娘有口上
            say_chara_line(chara, ctx, 'breast_caress')

    # 推进时间
    ctx.advance_time(command_time_data['breast_caress'])
    ...
    for actor_id in train.actors:
        ...
        chara = get_entity_by_id(world.player, actor_id)
        temp_sources[actor_id]['b_pleasure_source'] += chara.abl['finger_abl'] * 15
