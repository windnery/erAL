...
def can(world: World):
    ...
    if len(train_manager.train.actors) * 2 < len(train_manager.train.targets):  # type: ignore
        return False
    # 槽位判定（调教方1个手）
    if not check_body_slots(world, actor_slots={'hands': 1}):
        return False
...
def continuous_tick(world: World, ctx: CommandContext, cmd: ContinuousCommand):
    ...
    source: dict[str, int] = new_source({
        'c_pleasure_source': 20,
        'b_pleasure_source': 20,
        'love_source': 25,
        'sex_act_source': 30,
        'exposure_source': 10,
        'unclean_source': 15,
    ...
    for target_id in cmd.target_ids:
        ...
        # 50% 消耗
        ctx.consume(stamina=5, energy=20, chara=chara)
...
@register_cmd(
    ...
    needs_target=False,
    continuous=True,
    continuous_text='{actors}正在爱抚{targets}',
    actor_slots={'hands': 1},
    target_slots={},
...
def caress(world: World):
    ...
    if getattr(world, 'is_current_cmd_continuous', False):
        new_cmd = world.train_manager.add_continuous_cmd(
            'caress', list(train.actors), list(train.targets))
        if new_cmd:
    ...
    for target_id in train.targets:
        ...
        if target_id != PLAYER_ID:
            # 只有舰娘有口上
            say_chara_line(chara, ctx, 'caress')
