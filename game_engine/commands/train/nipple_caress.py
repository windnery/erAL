...
def continuous_tick(world: World, ctx: CommandContext, cmd: ContinuousCommand):
    ...
    for actor_id in cmd.actor_ids:
        ...
        # 50% 消耗
        ctx.consume(stamina=5, energy=10, chara=chara)
...
@register_cmd(
    ...
    needs_target=False,
    continuous=True,
    continuous_text='{actors}正在玩弄{targets}的乳头',
    actor_slots={'hands': 1},
...
def nipple_caress(world: World):
    ...
    if getattr(world, 'is_current_cmd_continuous', False):
        new_cmd = world.train_manager.add_continuous_cmd(
            'nipple_caress', list(train.actors), list(train.targets))
        if new_cmd:
