...
def orgasm_proc(orgasm_lv: dict[str, int], target: Character, orgasm_num: int):
    """绝顶处理"""
    mes: list[str] = []
...
def orgasm_palam_juel_proc(palam_id: str, target: Character, orgasm_num: int = 1, orgasm_lv: int = 1):
    """绝顶palam juel处理"""
    # 绝顶部位palam减半
...
def orgasm_check(target: Character):
    """绝顶检查"""
    mes = []
    orgasm_lv = {
        # 绝顶等级 最大4
